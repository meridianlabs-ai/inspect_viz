...
def text(
    ...
    text_anchor: TextAnchor | Param | None = None,
    line_height: float | Param | None = None,
    line_width: float | Param | None = None,
    text_overflow: TextOverflow | Param | None = None,
    monospace: bool | Param | None = None,
    font_family: str | Param | None = None,
    font_size: Channel | float | Param | None = None,
    font_variant: str | Param | None = None,
    font_weight: float | Param | None = None,
    **options: Unpack[MarkOptions],
    ...
    Args:
        ...
        line_height: The line height in ems; defaults to 1. The line height affects the (typically vertical) separation between adjacent baselines of text, as well as the separation between the text and its anchor point.
        line_width: The line width in ems (e.g., 10 for about 20 characters); defaults to infinity, disabling wrapping and clipping. If **textOverflow** is null, lines will be wrapped at the specified length. If a line is split at a soft hyphen (\xad), a hyphen (-) will be displayed at the end of the line. If **textOverflow** is not null, lines will be clipped according to the given strategy.
        text_overflow: How truncate (or wrap) lines of text longer than the given **lineWidth**; one of: null (default) - preserve overflowing characters (and wrap if needed); *clip* or *clip-end* - remove characters from the end; *clip-start* - remove characters from the start; *ellipsis* or *ellipsis-end* - replace characters from the end with an ellipsis (…); *ellipsis-start* - replace characters from the start with an ellipsis (…); *ellipsis-middle* - replace characters from the middle with an ellipsis (…). If no **title** was specified, if text requires truncation, a title containing the non-truncated text will be implicitly added.
        monospace: If true, changes the default **fontFamily** to *monospace*, and uses simplified monospaced text metrics calculations.
        font_family: The font-family; a constant; defaults to the plot's font family, which is typically *system-ui*.
        font_size: The font size in pixels; either a constant or a channel; defaults to the plot's font size, which is typically 10. When a number, it is interpreted as a constant; otherwise it is interpreted as a channel.
        font_variant: The font variant; a constant; if the **text** channel contains numbers or dates, defaults to *tabular-nums* to facilitate comparing numbers; otherwise defaults to the plot's font style, which is typically *normal*.
        font_weight: The font weight; a constant; defaults to the plot's font weight, which is typically *normal*.
