...
The [scores_timeline()](./reference/inspect_viz.view.html.md#scores_timeline) function plots eval scores by model, organization, and release date[^1]:
...
A [select()](./reference/inspect_viz.input.html.md#select) input for tasks is automatically provided if more than one task exists in the `data`. A [checkbox_group()](./reference/inspect_viz.input.html.md#checkbox_group) is automatically provided for organizations if more than one organization exists (you can disable this with `organizations_filter=False`).
...
The [Scores Timeline](./examples/inspect/scores-timeline/index.html.md) example demonstrates how this view was implemented using lower level plotting components.
