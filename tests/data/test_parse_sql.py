...
def test_sample_percentage() -> None:
    """Test handling of SAMPLE clause with percentage."""
    # SQL with SAMPLE as percentage
    sql: str = "SELECT category, product FROM products SAMPLE 25%"

    # Convert the SQL
    try:
        result: MosaicQuery = parse_sql(sql)

        # Verify that SAMPLE was recognized and converted to decimal
        assert result.sample == 0.25
        assert result.select
        assert "category" in result.select
        assert "product" in result.select
    except Exception as e:
        pytest.skip(f"Dialect doesn't support SAMPLE with percentage: {str(e)}")


def test_sample_rows() -> None:
    """Test handling of SAMPLE clause with row count."""
    # SQL with SAMPLE as row count
    sql: str = "SELECT id, name FROM users SAMPLE 100 ROWS"

    # Convert the SQL - the exact syntax depends on the dialect support
    try:
        result: MosaicQuery = parse_sql(sql)

        # Verify that SAMPLE was recognized as an integer count
        assert result.sample == 100
        assert result.select
        assert "id" in result.select
        assert "name" in result.select
    except Exception as e:
        # Skip test if the dialect doesn't support this syntax
        pytest.skip(f"Dialect doesn't support SAMPLE with row count: {str(e)}")
