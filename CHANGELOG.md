...
- Quarto: Optional `js+png` output mode embeds a static PNG fallback behind each plot so pages remain readable even if the JavaScript pipeline fails (e.g. CDN blocked, restrictive network). Recommended setup is a Quarto `publish` profile (`_environment-publish` with `INSPECT_VIZ_OUTPUT_FORMAT=js+png`) so production renders get the fallback while `quarto preview` stays fast.
